/*
*	Solution to Project Euler's Exercise #11
*	Author: Brei
*
...
void dispArr(const int arr[20][20])
{
	// Display all elements in 20 x 20 form
	for(size_t row = 0; row < 20; ++row) {

		for(size_t col = 0; col < 20; ++col) {
			cout << arr[row][col] << " ";
			
			// Form a newline if iteration is at the 20th column
			if(col == 19) {
				cout << endl;
			}
		}
...
int largestProduct(const int arr[20][20])
{
	...
	while(true) {
		current = arr[rowIdx][colIdx] * arr[rowIdx + 1][colIdx] * arr[rowIdx + 2][colIdx]
			* arr[rowIdx + 3][colIdx];

		
		cout << arr[rowIdx][colIdx] << " * " << arr[rowIdx + 1][colIdx] << " * " 
			<< arr[rowIdx + 2][colIdx] << " * " << arr[rowIdx + 3][colIdx] << " = " << current
			<< endl;
